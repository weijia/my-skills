...
当项目部署在子目录（如 `online/my-app/latest/`、`online/my-app/release/`）时，可添加页面底部切换链接，方便用户在版本间跳转。

### 何时需要

满足以下任一条件时建议添加：
- 项目同时维护稳定版和最新版
- 项目有开发版本（如日期目录 `20250525`）和正式版之分
- 用户需要在不同版本间对比或回退
...
| 条件 | 说明 |
|------|------|
| **子目录部署** | 页面部署在子目录下，非服务器根目录 |
| **release 目录存在** | `release/` 目录已创建（稳定版存放位置） |

### release 目录创建

如果 `release/` 目录不存在，可在 GitHub Actions workflow 中添加创建步骤：

```yaml
# 在 Deploy to WebDAV job 中添加
- name: Create release directory
  run: |
    curl -X MKCOL -u "${{ secrets.WEBDAV_USERNAME }}:${{ secrets.WEBDAV_PASSWORD }}" \
      "${{ secrets.WEBDAV_URL }}online/${{ env.PROJECT_NAME }}/release/"
```

### 实现逻辑

从 `window.location.pathname` 提取当前版本目录名（路径最后一段），生成相对路径链接：

| 当前版本目录 | 显示文字 | 链接 |
|-------------|---------|------|
| `latest` | 切换到正式版 | `../release/index.html` |
| `release` | 切换到最新版 | `../latest/index.html` |
| 其他（如日期 `20250525`） | 切换到最新版 | `../latest/index.html` |
...
{(() => {
  ...
  if (currentDir === 'latest') {
    return <a href="../release/index.html">切换到正式版</a>
  }
  if (currentDir === 'release' || /^\d{8}$/.test(currentDir)) {
    return <a href="../latest/index.html">切换到最新版</a>
  }
  return null
})()}
```
...
创建 `VersionSwitch.svelte` 组件：

```svelte
<script>
  import { isChineseLocale } from './i18n-utils.js'
...
(function() {
  ...
  if (currentDir === 'latest') {
    ...
    text = '切换到正式版';
  } else if (currentDir === 'release' || /^\d{8}$/.test(currentDir)) {
    link = '../latest/index.html';
    text = '切换到最新版';
  }

  if (link) {
    var el = document.getElementById('version-switch');
    el.innerHTML = '<a href="' + link + '">' + text + '</a>';
    el.style.cssText = 'position:fixed;bottom:8px;right:16px;z-index:1000;font-size:12px;';
  }
})();
</script>
```
...
- 使用**相对路径** `../` 而非绝对路径，因为项目部署在子目录下
- 从 pathname 最后一段提取版本目录名，与前面有多少层路径无关
- 日期格式（8位数字）自动识别为开发版本
